 ...
 */
public class RoundRectRule extends RectRule {
     ...
     */
    public RoundRectRule(Paint paint, String key, boolean drawOnFront, int gravity, float rx, float ry, RectF... values) {
    ...
    public void onDraw(DrawableLayout target, Canvas canvas) {
        ...
        if (animatedValue != null && !animatedValue.isEmpty())
            canvas.drawRoundRect(animatedValue, rx, ry, getPaint());
    }
}